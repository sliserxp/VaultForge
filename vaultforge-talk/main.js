...
var VaultForgeTalk = class extends import_obsidian.Plugin {
  async onload() {
    ...
    const core = this.app.plugins.getPlugin("vaultforge-core");
    const apiKey = core?.settings?.apiKey || this.settings.apiKey;
    if (!apiKey) {
      new import_obsidian.Notice("VaultForge-Talk: No API key found in Core or Talk settings.");
    } else {
      this.client = new openai_default({ apiKey, dangerouslyAllowBrowser: true });
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        new import_obsidian.Notice(`${npcName}: ${npcReply}`);
      }
    });
    this.registerView(
...
${context}`;
      ...
      console.error(err);
      new import_obsidian.Notice("Error generating NPC response.");
      return "\u2026";
    }
  }
  async saveTranscript(npcName, playerLine, npcReply) {
...
`);
      ...
      await vault.append(file, transcriptLine);
    }
...
var VaultForgeTalkView = class _VaultForgeTalkView extends import_obsidian.MarkdownView {
  static {
    this.VIEW_TYPE = "vaultforge-talk-view";
  }
  ...
  getViewType() {
    return _VaultForgeTalkView.VIEW_TYPE;
  }
  getDisplayText() {
...
var VaultForgeTalkSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Autoplay NPC Voice").setDesc("Automatically play NPC voice lines").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.autoplay).onChange(async (value) => {
        this.plugin.settings.autoplay = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Transcript Saving").setDesc("Where to save dialogue transcripts").addDropdown(
      (dropdown) => dropdown.addOptions({
        npc: "NPC Note",
        transcript: "Dedicated Transcript Note",
        both: "Both",
        none: "None"
      }).setValue(this.plugin.settings.transcriptMode).onChange(async (value) => {
