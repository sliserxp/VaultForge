...
/* ===============Utilities===================*/
  ...
  async function getTemplateChoices(app: App, folder: string): Promise<string[]> {
...
/*===============Modal================*/
  class InputModal extends Modal {
    ...
    constructor(app: App, titleText: string, body: string) {
      super(app);
...
/* ==============Main Plugin================ */
      ...
      callback: async () => {
        new InputModal(this.app, "Enter topic name (NPC, Faction, Item, Guild, etc.)", async (topic) => {
          ...
          try {
            ...
            if (!result?.status) {
              new Notice("ExpandContext: GPT did not return valid JSON (check console).");
              return;
            }
            ...
            if (result.status === "missing") {
              ...
              if (!templates.length) {
                new Notice("ExpandContext: No templates found in /Templates.");
                return;
              }
            ...
            new Notice("ExpandContext failed (see console).");
          }
        }).open();
      }
    });
  ...
  callback: async () => {
    if (!this.app) { new Notice("Error: app not available"); return; }

    new InputModal(this.app, "Ask GPT using Vault context", async (question) => {
      if (!question) return;

      try {
        const context = await this.askVault(
          `Provide all known details relevant to: ${question}`
        );
        ...
        new Notice("Error during chat with vault. Check console.");
      }
    }).open();
  },
});
