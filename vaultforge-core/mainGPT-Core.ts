...
const DEFAULT_SETTINGS: GPTCoreSettings = {
  apiKey: "",
  model: "gpt-4o",
...
/* ---------- Main Plugin ---------- */
export default class GPTCorePlugin extends Plugin {
  settings!: GPTCoreSettings;
  vaultIndex: VaultIndexEntry[] = [];
  openai: OpenAI | null = null;

  private log(...args: any[]) {   // ✅ only logs if dev mode is on
    if (this.settings?.developerMode) {
      console.log("[GPT-Core]", ...args);
    }
  }
    ...
    this.addCommand({
      ...
      callback: async () => {
        if (!this.settings.apiKey) {
          new Notice("⚠️ No API key set in GPT-Core settings.");
          return;
        }
        new Notice("✅ GPT-Core is active and settings are loaded!");
      },
    });
  ...
  async loadVaultIndex() {
    ...
    try {
      ...
      console.error("[GPT-Core] Failed to load index.json:", err);
      this.vaultIndex = [];
    }
  }
  ...
  async saveVaultIndex() {
    ...
    try {
      ...
      console.error("[GPT-Core] Failed to save index.json:", err);
    }
  }
async askVault(query: string): Promise<string> {
  console.log("=== [GPT-Core] askVault START ===");
  console.log("[GPT-Core] Query:", query);
  ...
  console.log("[GPT-Core] Direct matches found:", matches.length);

  /* ---------- 2. If no direct matches, use embeddings ---------- */
  if (matches.length === 0) {
    console.log("[GPT-Core] No direct match. Falling back to embeddings…");
    ...
    try {
      ...
      console.log("[GPT-Core] Embedding matches found:", matches.length);
    } catch (err) {
      console.error("[GPT-Core] Embedding search failed:", err);
    }
  }
...
${related}`;
  ...
  console.log("[GPT-Core] askVault returning context length:", results.length);
  console.log("=== [GPT-Core] askVault END ===");
  ...
  for (const file of files) {
    ...
    if (this.settings.indexGranularity === "chunk") {
      ...
      for (let i = 0; i < content.length; i += this.settings.chunkSize) {
        ...
        if (!oldEntry) {
          console.log(`[GPT-Core] Re-embedding chunk of ${file.path}`);
          const vecs = await this.embedMany([chunk]);
          embedding = vecs[0];
        } else {
          console.log(`[GPT-Core] Using cached embedding for chunk of ${file.path}`);
        }
      ...
      if (!oldEntry || oldEntry.lastHash !== fileHash) {
        ...
        console.log(`[GPT-Core] Using cached embedding for ${file.path}`);
      }
...
/* ---------- Settings Tab ---------- */
class GPTCoreSettingTab extends PluginSettingTab {
  plugin: GPTCorePlugin;
  constructor(app: App, plugin: GPTCorePlugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc("Enable verbose logging for debugging GPT-Core")
      .addToggle((toggle) =>
