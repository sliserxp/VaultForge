...
async function hashString(str2) {
...
var VaultForgeCorePlugin = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
    this.vaultIndex = [];
    this.openai = null;
    ...
    this.addCommand({
      ...
      callback: async () => {
        if (!this.settings.apiKey) {
          new import_obsidian.Notice("\u26A0\uFE0F No API key set in VaultForge-Core settings.");
          return;
        }
        new import_obsidian.Notice("\u2705 VaultForge-Core is active and settings are loaded!");
      }
    });
    this.addCommand({
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        if (q) {
          ...
          new import_obsidian.Notice(result.substring(0, 2e3));
  ...
  getIndexPath() {
    return (0, import_obsidian.normalizePath)(`${this.manifest.dir}/index.json`);
  }
  async loadVaultIndex() {
    ...
    try {
      const path2 = this.getIndexPath();
      if (await this.app.vault.adapter.exists(path2)) {
        const raw = await this.app.vault.adapter.read(path2);
  ...
  async saveVaultIndex() {
    ...
    try {
      const path2 = this.getIndexPath();
      await this.app.vault.adapter.write(path2, JSON.stringify(this.vaultIndex, null, 2));
  ...
  async askVault(query) {
    ...
    if (matches.length === 0) {
      ...
      try {
        ...
        matches = this.vaultIndex.filter((e) => e.embedding).map((entry) => ({
          ...entry,
          score: this.cosineSim(entry.embedding, qVec)
        })).sort((a, b) => b.score - a.score).slice(0, this.settings.maxResults);
    ...
    const results = matches.slice(0, this.settings.maxResults).map((entry) => {
      const yamlPreview = Object.entries(entry.yaml || {}).map(([k, v]) => `${k}: ${v}`).join(", ");
      const headingPreview = entry.headings?.slice(0, 3).join(" | ") || "";
      const excerpt = entry.text.substring(0, 500).replace(/\n+/g, " ");
      const related = entry.related?.length ? entry.related.map((r) => `- [[${r}]]`).join("\n") : "None";
      return `**${entry.path}**
YAML: ${yamlPreview}
...
${related}`;
  ...
  async chat(messages, modelOverride) {
    if (!this.openai) {
      new import_obsidian.Notice("\u26A0\uFE0F OpenAI not initialized. Check your API key.");
    ...
    try {
      ...
      new import_obsidian.Notice("\u274C Chat request failed. See console for details.");
      return "Error: " + err.message;
    }
  }
  async embedMany(texts) {
    if (!this.openai) throw new Error("OpenAI not initialized.");
    const cleanTexts = texts.map((t) => t.trim()).filter((t) => t.length > 0).map((t) => t.slice(0, 7e3));
  ...
  async buildVaultIndex() {
    ...
    for (const entry of newIndex) {
      entry.related = newIndex.filter((e) => e !== entry && e.tags.some((t) => entry.tags.includes(t))).map((e) => e.path.replace(/\.md$/, ""));
    }
    ...
    new import_obsidian.Notice(`\u2705 Vault index updated (${this.vaultIndex.length} entries).`);
...
var VaultForgeSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Chunk Size").setDesc("Max characters per chunk for indexing").addText(
      (text) => text.setPlaceholder("500").setValue((this.plugin.settings.chunkSize ?? 500).toString()).onChange(async (value) => {
        this.plugin.settings.chunkSize = parseInt(value) || 500;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Auto-Rebuild Index").setDesc("Automatically rebuild the index when files are modified").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.autoRebuild ?? true).onChange(async (value) => {
        this.plugin.settings.autoRebuild = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Use Local Index").setDesc("Cache embeddings in index.json to avoid re-sending unchanged files to the API").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.useLocalIndex ?? true).onChange(async (value) => {
        this.plugin.settings.useLocalIndex = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Index granularity").setDesc("Choose whether embeddings are per file (faster, less precise) or per chunk (slower, more detailed).").addDropdown((drop) => {
      ...
      drop.onChange(async (value) => {
        this.plugin.settings.indexGranularity = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian.Setting(containerEl).setName("Max Results").setDesc("Maximum number of matches to return from AskVault").addText(
      (text) => text.setPlaceholder("5").setValue((this.plugin.settings.maxResults ?? 5).toString()).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Embedding Model").setDesc("Choose which model to use for vault indexing").addDropdown((drop) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Default GPT Model").setDesc("Choose which GPT model to use for chat responses").addDropdown((drop) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Developer Mode").setDesc("Enable verbose logging for debugging VaultForge-Core").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.developerMode ?? false).onChange(async (value) => {
