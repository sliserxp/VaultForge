...
import { GPTCoreAPI } from "./type"; // ✅ type-safe Core API

/* ======================= Interfaces & Defaults ======================= */
interface GPTChatSettings {
  chatFolder: string;
...
const DEFAULT_SETTINGS: GPTChatSettings = {
  chatFolder: "Chats",
...
export default class GPTChatPlugin extends Plugin {
  core: GPTCoreAPI | null = null;
  settings!: GPTChatSettings;

  /* ---------- Lifecycle ---------- */
  async onload() {
    console.log("Loading GPT-Chat...");
    await this.loadSettings();

    this.core = (this.app as any).plugins.getPlugin("gpt-core") as GPTCoreAPI | null;
    if (!this.core) {
      new Notice("⚠️ GPT-Core not loaded. GPT-Chat disabled.");
      return;
    }
    ...
    console.log("✅ GPT-Chat loaded and linked to GPT-Core");
  }

  onunload() {
    console.log("Unloading GPT-Chat...");
  }
  ...
  async handleNoteUpdate(file: TFile) {
    ...
    if (this.settings.useVaultContext && this.core) {
      try {
        console.log("=== [GPT-Chat] Vault context lookup START ===");
  ...
  vaultContext = await this.core.askVaultConcise(userMessage);
        ...
        console.log(
          "[GPT-Chat] Vault context retrieved:",
          vaultContext.length > 300 ? vaultContext.slice(0, 300) + "..." : vaultContext
        );
        console.log("=== [GPT-Chat] Vault context lookup END ===");
      } catch (err) {
...
class GPTChatSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    containerEl.createEl("h2", { text: "GPT-Chat Settings" });
